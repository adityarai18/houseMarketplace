...
function EditListing() {
  ...
  const [listing, setListng] = useState(null);
  ...
  const onSubmit = async (e) => {
    ...
    if (images.length > 6) {
      setLoading(false);
      toast.error('Maximum number of images must be 6');
      return;
    }
    ...
    if (geolocationEnabled) {
      const response = await fetch(
        `http://api.positionstack.com/v1/forward?access_key=${process.env.REACT_APP_ACCESS_KEY}&query=${address}`
      );
